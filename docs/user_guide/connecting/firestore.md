...
For local development, you can also connect to the Firebase Emulator instead of a live Google Cloud project.
...
Beekeeper Studio supports three authentication methods for Firestore:
...
3. **Firebase Emulator** — connects to a local Firestore emulator without a service account key

### Connection Settings

| Field                     | Required | Description                                                     |
| ------------------------- | -------- | --------------------------------------------------------------- |
| Authentication Method     | Yes      | Service Account Key, ADC, or Firebase Emulator                  |
...
| Emulator Host             | No\*\*   | Firestore emulator host and port, such as `localhost:8080`      |
...
\*\*Used only when Authentication Method is set to Firebase Emulator.
...
When using Firebase Emulator, set the Firestore emulator host in Beekeeper Studio, for example `localhost:8080`. Firebase Auth uses the same hostname on port `9099`.
...
Firebase Auth management depends on Firebase Auth being available for the current connection. Emulator connections use the same hostname as the Firestore emulator and connect to Firebase Auth on port `9099`.
