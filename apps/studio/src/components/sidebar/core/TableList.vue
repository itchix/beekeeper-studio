<template>
  <div class="flex-col expand table-list-component" ref="wrapper">
    <!-- Filter -->
    <div class="fixed">
      <div class="filter">
    ...
    <!-- Tables -->
    <hr v-show="pinnedEntities.length > 0" />
    ...
    <nav class="list-group flex-col" ref="tables">
      <div class="list-heading">
        ...
        <span :title="`Total Entities`" class="badge" v-if="!filterQuery">{{
          totalEntities
        }}</span>
        <span
        ...
        <div class="actions">
          ...
          <x-button v-if="!usedConfig?.readOnlyMode" class="settings-btn" menu>
            <i class="material-icons">add</i>
            <i class="material-icons">arrow_drop_down</i>
            <x-menu>
              <x-menuitem :disabled="tablesLoading" @click.prevent="newTable">
                <x-label>
...
export default {
  ...
  computed: {
    ...
    totalEntities() {
      return (
        this.tables.length + this.routines.length - this.hiddenEntities.length
      );
    },
    shownEntities() {
      ...
      return [
        {
          ...
          handler: this.togglePinTableList,
        },
      ];
    },
  },
  ...
  methods: {
    ...
    openAuthTab() {
      this.$root.$emit(AppEvent.openFirestoreAuthTab);
    },
