  ...
  computed: {
    ...
    rootBindings() {
      return [
        ...
        {
          event: AppEvent.openFirestoreAuthTab,
          handler: this.openFirestoreAuth,
        },
  ...
  methods: {
    ...
    async createTab(config: TabTypeConfig.Config) {
      if (config.type === "query") {
        ...
        config.type === "plugin-shell" ||
        config.type === "plugin-base"
      ) {
        let tNum = 0;
    ...
    async handlePromptQueryExport(query) {
      ...
      const lastExportPath = await Vue.prototype.$settings.get(
        "lastExportPath",
        await window.main.defaultExportPath(fileName)
      );
    ...
    copyName(item) {
      if (item.tabType !== "table" && item.tabType !== "table-properties")
        return;
