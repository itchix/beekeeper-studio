  ...
  >
    <div class="top-panel" ref="topPanel">
      ...
      <div class="toolbar text-right" ref="toolbar">
              ...
              <x-button
                ...
                v-tooltip="
                  getCommitModeVTooltip({
                    ...
                    description:
                      'This is the way it works by default. No need to worry about it.',
                  })
                "
              >
              ...
              <x-button
                ...
                v-tooltip="
                  getCommitModeVTooltip({
                    ...
                    description:
                      'Write actions will require you to manually commit your changes',
                    learnMoreLink:
                      'https://docs.beekeeperstudio.io/user_guide/sql_editor/manual-transaction-management',
                  })
                "
              >
                <span class="togglebutton-content">
            ...
            <div
              ...
              v-tooltip="{
                ...getCommitModeVTooltip({
                  ...
                  description:
                    'Once committed or rolled back, it will be deactivated.',
                  learnMoreLink:
                    'https://docs.beekeeperstudio.io/user_guide/sql_editor/manual-transaction-management',
        ...
        <div
          v-if="canManageTransactions && isManualCommit"
        ...
        <div class="actions btn-group">
          ...
          >
            ...
            <i v-if="isCommunity" class="material-icons menu-icon">stars</i>
            ...
            >
              ...
              <x-menu>
                <x-menuitem @click.prevent="queryFunctions.primaryRead">
                  <x-label>{{ runPrimaryText() }}</x-label>
                  <x-shortcut
                    :value="displayShortcut('queryEditor.primaryQueryAction')"
                  />
                </x-menuitem>
                <x-menuitem @click.prevent="queryFunctions.secondaryRead">
                  <x-label>{{ runSecondaryText() }}</x-label>
                  <x-shortcut
                    :value="displayShortcut('queryEditor.secondaryQueryAction')"
                  />
                </x-menuitem>
                <hr />
                <x-menuitem
                ...
                >
                  ...
                  <x-shortcut
                    :value="
                      displayShortcut('queryEditor.primaryQueryToFileAction')
                    "
                  />
                  <i v-if="isCommunity" class="material-icons menu-icon">
                    stars
                  </i>
                </x-menuitem>
                <x-menuitem
                ...
                >
                  ...
                  <x-shortcut
                    :value="
                      displayShortcut('queryEditor.secondaryQueryToFileAction')
                    "
                  />
                  <i v-if="isCommunity" class="material-icons menu-icon">
                    stars
    ...
    <portal to="modals">
      <modal
        ...
        :scrollable="true"
      >
        <form v-kbd-trap="true" v-if="query" @submit.prevent="saveQuery">
          <div class="dialog-content">
            ...
            <div class="modal-form">
              ...
              <div
                class="form-group"
                v-if="queryFolders && queryFolders.length > 0"
              >
                <label
                  >Folder
                  <i
                    v-if="!isUltimate && !isCloud"
        ...
        >
          <div class="dialog-content">
            ...
            <div class="dialog-c-subtitle">
              You need to use single quotes around string values. Blank values
              are invalid
            </div>
            <div class="modal-form">
              <div class="form-group">
...
export default {
  ...
  computed: {
    ...
    editorComponent() {
      return this.connectionType === "surrealdb"
        ? SurrealTextEditor
        : SqlTextEditor;
    },
    enabled() {
    ...
    runningText() {
      return `Running ${this.runningType} (${window.main.pluralize(
        "query",
        this.runningCount,
        true
      )})`;
    ...
    hasTitle() {
      return (
        this.query?.title && this.query.title.replace(/\s+/, "").length > 0
      );
    },
    splitElements() {
    ...
    keymap() {
      ...
      return this.$vHotkeyKeymap({
        ...
        "queryEditor.primaryQueryToFileAction":
          this.queryFunctions.primaryWrite,
        "queryEditor.secondaryQueryToFileAction":
          this.queryFunctions.secondaryWrite,
    ...
    queryParameterPlaceholders() {
      ...
      if (
        ...
        (this.hasSelectedText || this.runningType === "current")
      ) {
    ...
    deparameterizedQuery() {
      ...
      try {
        const placeholders = this.individualQueries.flatMap(
          (qs) => qs.parameters
        );
        ...
        const convertedParams = convertParamsForReplacement(
          placeholders,
          values
        );
        query = deparameterizeQuery(
          query,
          this.dialect,
          convertedParams,
          this.paramTypes
    ...
    vimConfig() {
      const exCommands = [
        ...
        { name: "quit", prefix: "q", handler: this.close },
        {
          name: "qa",
          prefix: "qa",
          handler: () => this.$root.$emit(AppEvent.closeAllTabs),
        },
    ...
    paramTypes() {
      ...
      const dbType =
        this.connectionType === "postgresql" ? "postgres" : this.connectionType;
    ...
    primaryIsTab() {
      return (
        this.$bksConfig.ui.queryEditor?.primaryQueryAction.toLowerCase() ===
        "submittabquery"
      );
    },
    primaryIsCurrent() {
  ...
  methods: {
    ...
    getPresets(presetId) {
      this.$util
        .send("appdb/formatter/getAll")
        .then((presets) => {
          const presetToFind =
            typeof presetId === "object"
              ? this.$bksConfig.ui.queryEditor.defaultFormatter
              : presetId;
          const selectedFormatter = presets.find(
            (p) => Number(p.id) === Number(presetToFind)
          );

          if (selectedFormatter != null)
            this.selectedFormatter = {
              id: selectedFormatter.id,
              ...selectedFormatter.config,
            };
    ...
    async deletePreset({ id }) {
      if (
        !(await this.$confirm(
          "Are you sure you want to delete this configuation?"
        ))
      ) {
        return;
      }
      ...
      this.$util
        .send("appdb/formatter/deletePreset", { id })
        .then(() => {
        ...
        .catch((err) => {
          const error_notice = this.$noty
            .error(`Formatter Configuration delete failed: ${err.message}`, {
              buttons: [
    ...
    savePreset({ id, config, name }) {
      ...
      this.$util
        ...
        .catch((err) => {
          const error_notice = this.$noty
            .error(`${notyMessage} failed: ${err.message}`, {
              buttons: [
                Noty.button("Close", "btn btn-primary", () => {
                  error_notice.close();
                }),
    ...
    async editResults() {
      ...
      if (this.isCommunity) {
        this.$root.$emit(
          AppEvent.upgradeModal,
          "Upgrade required to edit query result data"
        );
        return;
      }
      if (!this.resultsEditData[this.selectedResult]) {
        const resultEditData: FieldEditData[] =
          await this.connection.getResultEditData(
            this.result?.text,
            this.result.fields
          );
        ...
        if (!resultEditData.some((e) => e.editable)) {
          this.$noty.warning(
            "Editing results cannot be enabled because no primary keys are included in the query",
            {
              buttons: [
                Noty.button("Learn More", "btn btn-primary", () => {
                  window.main.openExternally(
                    "https://beekeeperstudio.io/user_guide_sql_editor/editing-data.md"
                  );
                }),
              ],
            }
    ...
    async saveQuery() {
      ...
      if (!this.hasTitle || !this.hasText) {
        ...
        try {
          ...
          this.tab.queryId = id;
          try {
            const savedQuery = await this.$store.dispatch(
              "data/queries/findOne",
              id
            );
    ...
    addTransactionTimeoutListener() {
      const connectionType =
        this.connectionType === "postgresql" ? "postgres" : this.connectionType;
      this.transactionTimeoutWarningListenerId = this.$util.addListener(
        `transactionTimeoutWarning/${this.tab.id}`,
        () => {
          this.showKeepAlive = true;
          this.warningNoty = this.$noty.warning(
            `The Transaction in ${
              this.tab?.query?.title ?? this.tab?.title
            } will be automatically rolled back soon!`,
            {
              buttons: [
              ...
              timeout:
                this.$bksConfig.db[connectionType].autoRollbackWarningWindow,
            }
          );
        }
      );

      this.transactionTimeoutListenerId = this.$util.addListener(
        `transactionTimedOut/${this.tab.id}`,
        () => {
    ...
    async submitQueryToFile() {
      ...
      const query_sql =
        this.hasSelectedText && this.primaryIsTab
    ...
    async submitCurrentQuery() {
      ...
      const queries = identify(this.unsavedText, {
        strict: false,
        dialect: this.identifierDialect,
    ...
    async submitQuery(rawQuery, fromModal = false) {
      ...
      try {
        ...
        if (
          this.canManageTransactions &&
          identification.some(
            (value: IdentifyResult) => value.executionType === "TRANSACTION"
          )
        ) {
          const startTransaction = identification.filter(
            (value: IdentifyResult) => value.type === "BEGIN_TRANSACTION"
          ).length;
          const endTransaction = identification.filter(
            (value: IdentifyResult) =>
              value.type === "COMMIT" || value.type === "ROLLBACK"
          ).length;
      ...
      try {
        ...
        this.runningQuery = await this.connection.query(
          query,
          this.tab.id,
          { dryRun: this.dryRun },
          this.hasActiveTransaction
    ...
    initializeQueries() {
      ...
      if (originalText) {
        const queries = identify(originalText, {
          strict: false,
          dialect: this.identifierDialect,
          paramTypes: this.paramTypes,
        });
    ...
    async switchPaneFocus(
      ...
      if (target) {
        ...
        this.focusElement =
          this.focusElement === "text-editor" ? "table" : "text-editor";
      }
    },
    blurTextEditor() {
      let timedOut = false;
    ...
    async toggleCommitMode(mode?: "manual" | "auto") {
      ...
      if (_.isNil(mode)) {
        ...
        (mode === "manual" && this.isManualCommit) ||
    ...
    async keepAliveTransaction() {
      this.showKeepAlive = false;
      this.maybeCloseWarningNoty();
      await this.$util.send("conn/resetTransactionTimeout", {
        tabId: this.tab.id,
      });
    },
    async columnsGetter(tableName: string) {
      let table = this.tables.find(
        (t: TableOrView) =>
          t.name === tableName || `${t.schema}.${t.name}` === tableName
      );
    ...
    displayShortcut(shortcutPath: KeybindingPath) {
      const keybindings = this.$bksConfig.getKeybindings(
        "context-menu",
        shortcutPath
      );
      const displayKeybinding: string = Array.isArray(keybindings)
        ? keybindings[0]
        : keybindings;
          ...
          <div class="actions">
            ${
              options.learnMoreLink
                ? `<a href="${options.learnMoreLink}" class="btn btn-flat">Learn more</a>`
                : ""
            }
            ${
              options.show
                ? `<button class="btn btn-flat" data-close="true">Close</button>`
                : ""
            }
          </div>
        `;
...
<div class="commit-mode-tooltip-content ${options.className || ""}">
  ...
  async mounted() {
    ...
    if (this.tab.queryId) {
      this.fullQuery = await this.$store.dispatch(
        "data/queries/findOne",
        this.tab.queryId
      );
    } else if (this.tab.usedQueryId) {
      this.fullQuery = await this.$store.dispatch(
        "data/usedQueries/findOne",
        this.tab.usedQueryId
      );
    }
    this.initializeQueries();
