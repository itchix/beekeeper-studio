...
export function emptyResult(value: any) {
  if (typeof value === "undefined") {
    return buildNullValue("UNDEFINED");
  }
...
export function buildFormatterWithTooltip(
  cellValue: string,
  tooltip: string,
  icon?: string
) {
  if (!icon) {
    return `<div class="cell-link-wrapper" title="${escapeHtml(
      tooltip
    )}">${escapeHtml(cellValue)}</div>`;
  }

  return `<div class="cell-link-wrapper">${escapeHtml(
    cellValue
  )}<i class="material-icons fk-link" title="${escapeHtml(
    tooltip
  )}">${escapeHtml(icon)}</i></div>`;
}
...
export default {
  methods: {
    ...
    pillFormatter(cell: CellComponent) {
      ...
      return cellValue
        .map((cv) => `<span class="mapper-pill">${cv}</span>`)
        .join("");
    },
    cellTooltip(_event, cell: CellComponent) {
      const params: TabulatorFormatterParams =
        cell.getColumn().getDefinition().formatterParams || {};
      let cellValue = cell.getValue();

      if (cellValue instanceof Uint8Array) {
        const binaryEncoding = params.binaryEncoding || "hex";
        cellValue = `${_.truncate(
          this.niceString(cellValue, false, binaryEncoding),
          { length: 15 }
        )} (as ${binaryEncoding} string)`;
      } else if (
        !params?.fk &&
        !params?.isPK &&
      ...
      return nullValue
        ? nullValue
        : escapeHtml(this.niceString(cellValue, true));
    },
    cellFormatter(
      cell: CellComponent,
      params: {
      ...
      if (params?.fk) {
        if (params.fk.length === 1)
          tooltip = `View record in ${params.fk[0].toTable}`;
        else
          tooltip = `View records in ${(
            params.fk.map((item) => item.toTable).join(", ") as string
          ).replace(/, (?![\s\S]*, )/, ", or ")}`;
        ...
        onRendered(() => {
          const fkLink = cell
            .getElement()
            .querySelector(".fk-link") as HTMLElement;
