...
const store = new Vuex.Store<State>({
  ...
  actions: {
    async test(context, config: IConnection) {
      ...
      await Vue.prototype.$util.send("conn/test", {
    ...
    updateWindowTitle(context) {
      ...
      if (context.getters.isTrial && context.getters.isUltimate) {
        ...
        title += ` - Free Trial (${window.main.pluralize(
          "day",
          days,
          true
        )} left)`;
    ...
    async connect(
      ...
      if (context.state.username) {
        ...
        config = await context.dispatch(
          "data/usedconnections/recordUsed",
          resolvedConfig
        );
