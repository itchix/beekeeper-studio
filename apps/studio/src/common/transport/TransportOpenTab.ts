...
export type CoreTabType = 'query' | 'table' | 'table-properties' | 'settings' | 'table-builder' | 'backup' | 'import-export-database' | 'restore' | 'import-table' | 'shell' | 'firestore-auth'
...
export function matches(obj: TransportOpenTab, other: TransportOpenTab): boolean {
  ...
  switch (other.tabType) {
    ...
    case 'import-table':
      ...
      (obj.schemaName || null) === (other.schemaName || null);
    case 'firestore-auth':
      return obj.tabType === 'firestore-auth';
    default:
      return false
  }
}
